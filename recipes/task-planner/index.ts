...
const outputSchema = z.object({
  summary: z.string(),
  items: z.array(z.object({ title: z.string(), priority: z.string(), detail: z.string() })),
  nextSteps: z.array(z.string())
});

export const taskPlannerRecipe: Recipe<z.infer<typeof inputSchema>, z.infer<typeof outputSchema>> = {
  id: "task-planner",
  name: "Task Planner",
  ...
  async run(input, ctx) {
    const result = await generateStructured<z.infer<typeof outputSchema>>(
      [
        { role: "system", content: "Return JSON with summary, items, and nextSteps for a pragmatic task plan." },
