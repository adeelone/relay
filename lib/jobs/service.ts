...
export async function createJob(request: Request) {
  ...
  if (!rate.allowed) {
    return { error: "rate_limited", retryAfterSeconds: rate.retryAfterSeconds as number };
  }
  ...
  const event = await memoryDb.addEvent(job.id, { status: "queued", message: "Job queued" });
...
export async function getJobSnapshot(jobId: string) {
  ...
  const [events, attempts] = await Promise.all([memoryDb.listEvents(jobId), memoryDb.listAttempts(jobId)]);
...
export async function cancelJob(jobId: string) {
  ...
  const updated = await memoryDb.updateJob(jobId, { status: "cancelled", cancelledAt: new Date().toISOString() });
  const event = await memoryDb.addEvent(jobId, { status: "cancelled", message: "Cancellation requested" });
  publishJobEvent(event);
  return updated;
}
...
export async function retryJob(jobId: string) {
  ...
  const updated = await memoryDb.updateJob(jobId, { status: "queued", error: undefined, queuedAt: new Date().toISOString() });
  const event = await memoryDb.addEvent(jobId, { status: "queued", message: "Job re-queued" });
  publishJobEvent(event);
  setTimeout(() => {
